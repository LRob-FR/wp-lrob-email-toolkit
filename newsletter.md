...
- Default: only **failures** are logged to `wp_lrob_etk_logs`. Success rows live in `wp_lrob_etk_nl_newsletter_recipients` only. The Logging module reads `X-Lrob-Etk-Newsletter-ID` and skips writing a log row when the send succeeded.
- Per-newsletter override `_lrob_etk_nl_log_all_sends=true` logs every send for that newsletter.
...
- Optional one-row-per-newsletter summary log on completion (off by default; admin opt-in via `lrob_etk_nl_log_newsletter_summary`).
...
8. **Logging integration**: header tagging (`X-Lrob-Etk-Newsletter-ID` already shipped from SendLoop), Logging-module filter for newsletter, "Log all sends" override.
