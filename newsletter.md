...
### ✅ Step 7c (SMTP circuit-breaker + Retry-failed)

Defensive layer around `SendLoop` so a misconfigured SMTP server doesn't burn through every pending recipient in seconds.

- **Schema bump v7** — `wp_lrob_etk_nl_newsletters` grows `pause_reason varchar(50) DEFAULT NULL`. Additive; dbDelta picks it up via `Schema::install()` triggered by the `db_version_int` bump from 6 → 7.
- **SendLoop circuit-breaker** — new `CONSECUTIVE_FAILURE_THRESHOLD = 5`. The tick foreach tracks consecutive `wp_mail` failures, resets to 0 on every success, and bails out at threshold. On bail-out: any rows that were claimed-but-not-processed (already flipped to `sending` by `claim_batch`) get released back to `pending` via `release_claimed()`; status flips to `paused` with `pause_reason = 'smtp_unhealthy'`; `newsletter.paused` event fires with the reason. Resume clears `pause_reason` — if SMTP is still broken, the next tick will trip the breaker again.
- **Bad-recipient-data carve-out** — rows that fail with `'invalid_recipient_or_body'` count as failed but don't bump the consecutive-failure counter. A batch with a few invalid emails interspersed with a healthy SMTP server won't trip the breaker.
- **Admin banner** — when `is_paused && pause_reason === smtp_unhealthy`, the card renders a red `.lrob-etk-nl-card-banner.is-error` explaining the threshold and pointing at Resume.
- **Retry-failed** — new AJAX action `lrob_etk_nl_retry_failed` + `NewsletterRepository::retry_failed_recipients()` flips every `failed` row back to `pending`, clears `failure_code`, decrements `failed_count` on the companion (with `GREATEST(0, …)` guard against underflow). If the newsletter had already converged to `sent` or `failed`, status flips back to `sending` so SendCron picks up the re-queued rows. UI: `Retry failed (N)` button on each card that has `failed_count > 0`, confirmed via `etkConfirm`.
