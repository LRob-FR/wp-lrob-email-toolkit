...
6. ✅ **Campaign CPT + composer**. `CampaignCPT` registers `lrob_etk_nl_campaign` (19 chars, fits 20-char limit), Gutenberg-enabled, same constrained block subset as TemplateCPT. Plural-caps mapping pattern. `register_meta` for: `_lrob_etk_nl_preview_text`, `_from_name_override`, `_reply_to_override`, `_smtp_identity_id`, `_category_id`, `_target_spec` (JSON: `{kind, list_id?}`), `_scheduled_at` (UTC), `_track_opens`, `_track_clicks`, `_log_all_sends`. `CampaignMetaboxes` ships four boxes (Sender & preview / Audience / Schedule / Tracking) — vanilla PHP/JS, no JSX build. SMTP identity picker pulls from the SMTP module's `IdentityRepository` when available (gracefully empty when SMTP is disabled). `CampaignRepository` for the companion table: `ensure_row` (INSERT IGNORE on save), `update_status`, `delete_for_post` (cleans companion + campaign_recipients on post delete), `list_all` (JOIN with wp_posts for the admin view). `CampaignsPage` admin view ships at `?page=lrob-etk-nl&view=campaigns` (replaces the placeholder). Companion `status` flips between draft ↔ scheduled based on whether `_scheduled_at` is set, but only while the campaign is still pre-send — the send pipeline (step 7) drives it from there.
7. ✅ **Minimum viable send pipeline.** `src/Modules/Newsletter/Send/` package: `CampaignRenderer` (do_blocks + per-recipient token substitute: `{{email}}`, `{{name}}`, `{{first_name}}`, `{{prefs_url}}`, `{{unsub_url}}`, `{{site_name}}`, `{{site_url}}`); `Materializer` (resolves target_spec → `campaign_recipients` rows in chunks of 50, applies category opt-out filter, populates email/name snapshots + domain, INSERT IGNORE for idempotency, flips status to `sending` + fires `newsletter.campaign.started`); `SendLoop` (claim N pending → wp_mail each → flip row status → bump campaign counters; completion detection flips `sent` + dispatches `newsletter.campaign.completed`; List-Unsubscribe + List-Unsubscribe-Post headers on every send); `SendAjaxController` with two endpoints: `lrob_etk_nl_send_tick` (materialize-if-needed + tick + return progress; editor JS loops until `remaining === 0`) and `lrob_etk_nl_test_send` (renders + sends to current admin / ad-hoc address / configured test list; doesn't write `campaign_recipients` or touch counters; flagged with `X-Lrob-Etk-Newsletter-Test: 1`). New "Send" metabox at the top of the campaign editor sidebar with Send-now button, progress bar (sent/failed/total), and Test-send `<details>` panel.

### 🚧 Deferred to step 7b

- **WP-Cron safety net** (every-minute tick) — picks up `sending` campaigns whose `last_tick_at` is stale (>2 min) and processes one batch per campaign per tick.
...
- **Pause / resume / abort** — admin actions that flip the companion status; both AJAX and Cron honour `paused`/`aborted`.
