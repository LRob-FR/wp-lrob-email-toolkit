...
- **WP-Cron health diagnostic on the Newsletters page.** The send pipeline depends on `SendCron` (1-minute interval) firing for two things: triggering scheduled-at sends, and resuming a send the admin started in-browser then walked away from. WP's pseudo-cron only fires when traffic hits the site AND the loopback HTTP request to `wp-cron.php` succeeds; many self-hosted setups (NAT loopback issues, slow servers, `DISABLE_WP_CRON`) break this silently. Build a small diagnostic panel at the bottom of `&view=newsletters` (and maybe the Dashboard) that surfaces: the timestamp of the last observed `SendCron::handle_tick()` run (persisted to a `lrob_etk_nl_last_cron_tick` option from inside handle_tick), the next scheduled fire time (via `wp_next_scheduled(SendCron::CRON_HOOK)`), whether `DISABLE_WP_CRON` is set, and a colour-coded health badge: green = ticked in last ~3 min, yellow = 3–15 min, red = >15 min or never. When red/yellow, link to a short doc explaining the system-cron / external-cron-job fix. Optional: a "Run send tick now" button that posts to a thin AJAX wrapper around `SendCron::handle_tick()` for manual recovery — bounded by the existing per-tick budget so it doesn't lock the page.
