...
**The hard rule — `docs/todo.md` holds ONLY not-yet-done work; anything done goes to `docs/done.md`.** Never leave a done item in the todo — not even flagged "✅/DONE/BUILT/SHIPPED". "Done" means *built and functional* (it does **not** require a release/tag — un-tagged-but-working work still moves to done.md immediately). The moment something is implemented, in the SAME change: delete its entry (and any status flags) from `docs/todo.md` and write it into `docs/done.md`. Keep the two in sync as you work — when a backlog item gains/loses scope, update both. A done line (or a "BUILT/pending-test" marker) still sitting in the todo is a bug in the docs. *(The only status markers allowed in `docs/todo.md` are on the **milestone roadmap headers** — e.g. "v0.5.x — ✅ SHIPPED" — as navigation; the done work-items themselves live in `docs/done.md`, never as todo bullets.)*

### Doc-sync timing — non-negotiable

- **`docs/todo.md` + `docs/done.md` — reconcile the instant a change is done (built + functional), not only when the user validates or when it's released.** In that moment, move its line out of `docs/todo.md` into `docs/done.md`. Don't batch it to "later"; a done change still sitting in the todo is a stale doc.
...
- **Big per-editor cards live in a dedicated `*CardRenderer` class**, not inline in the page. The page keeps chrome (header/tabs/modals/notices) + admin-post + AJAX handlers + the render loop; it constructs the renderer once and delegates each card to `->render(...)`. Helpers a page *modal* also uses stay `public static` on the page (call `PageClass::x()` from the renderer); card-only helpers move into the renderer. Examples: SMTP `IdentityCardRenderer`, Newsletter `NewsletterCardRenderer` + `SignupFormCardRenderer`, Contact Form `FormCardRenderer`. **When extracting one, audit the moved body for BOTH `self::CONST`/`self::method` (→ which class owns it?) AND every `$this->prop` (→ pass it to the renderer's constructor) — `php -l` won't catch an undefined constant or a null property at runtime.**
- **Per-page card JS is an external file** enqueued by the module's `PageController::enqueue_assets()` + fed via `wp_localize_script` (e.g. `admin/js/smtp-cards.js` ↔ `lrobEtkSmtp`, `newsletter-cards.js` ↔ `lrobEtkNlSend`). No inline `<script>` heredocs in page classes.
...
**Card-edited CPTs have no Gutenberg editor** (`supports` omits `'editor'`) — they're built/edited entirely on a custom admin page. Their default WP screens (`edit.php` list, `post-new.php` create, `post.php?action=edit`) are therefore obsolete bare-title screens. The CPT class must redirect **all three** (idle GETs only; let bulk/save POSTs pass) to the custom page so a stray bookmark or Gutenberg back-arrow never lands on a dead screen — see `ContactForm\Admin\PageController::redirect_legacy_cpt_screens()` and `Newsletter\FormCPT::redirect_legacy_cpt_screens()`. CPTs that *do* keep `'editor'` for body content (newsletters, templates) redirect only the `edit.php` list, never the editor screens. **Never link to `post-new.php`/`post.php` for a card-edited CPT** — point at the custom page (creation goes through the in-page "New form" picker).
