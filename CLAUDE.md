...
- **Newsletter list polish — preview / recipients view / sub-tabs / duplicate-options dialog.** Once the inline-cards refactor lands, follow-ups: (1) **Preview button** per card — modal showing the rendered HTML as the current admin would see it; reuses NewsletterRenderer; new AJAX endpoint that returns HTML rather than sending. (2) **Per-newsletter recipients view** — drawer or sub-page exposing `newsletter_recipients` rows (recipient_kind, email_snapshot, status, sent_at, failure_code). Data's already there. Cross-link to Logging entries once step 8 ships. (3) **Sub-tabs on the newsletters list**: "In preparation" (draft/scheduled) vs "Sent" (sending/sent/aborted/failed/paused) so a long history doesn't drown out current work. (4) **Duplicate options dialog** — confirm modal asking "clone everything" (current behaviour) vs "just the content" (fresh settings on the copy) before performing the duplicate. (5) **Lock after sent** is already in the refactor; this group is the layered polish on top.
...
- **SMTP failure circuit-breaker for newsletter sends.** Today `SendLoop` calls `wp_mail()` per recipient and just marks each failure with `failure_code = 'wp_mail_failed'` then moves to the next — if the SMTP server is down, a 10k-recipient send burns through every pending row in seconds and the admin has no signal until they check the recipients drawer. Need: detect consecutive failures inside a single tick (≥N in a row, default N=5), or a failure ratio across the batch (≥50% failed), → flip companion status to `paused` with a new `pause_reason = 'smtp_unhealthy'` column on the companion row (or store in meta), → admin notice on the Newsletters view, → keep pending rows as `pending` (NOT flipped to `failed`) so they're picked up on resume, → manual "Resume" on the card or a "Retry connection now" button that runs a single test send before unpausing. Companion should also expose a `consecutive_failures` counter that resets to 0 on any successful send so transient blips don't trip the breaker. Pairs with the per-identity throttle entry below; both gate `SendLoop::tick()` before claiming the next batch. Also revisit: the failed rows from a burn-through (before the breaker landed) should be **retryable in bulk** — a "Retry failed (N)" button on each newsletter card that flips the matching `newsletter_recipients` rows back to `pending` and clears their `failure_code`.
