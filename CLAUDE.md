...
- **Newsletter statistics surfaces — dedicated view + dashboard tiles.** Step 9 records the data (per-newsletter aggregate counters, per-subscriber lifetime stats, per-event `tracking_events` rows, per-newsletter asset/link side-tables); v0.3.2 only surfaces opens + clicks on the card stats line. Need: (1) **Newsletter hub Statistics view** — sortable by date / open rate / click rate, charts for the trend over time (sends-per-month, average open rate over time, etc.), per-list rollups (`COUNT(*) WHERE list_id=? AND last_engagement_at >= NOW() - INTERVAL 90 DAY`, transient-cached 5min per CLAUDE.md "Per-list / per-category rollups"), per-category rollups, top-engaged subscribers vs cold subscribers split. Per-newsletter drill-down: per-asset opens (which images got loaded most), per-link clicks (which CTA performed), per-domain breakdown (which mail providers engaged). (2) **Email Toolkit dashboard tiles** — last N days total sends + total opens + total clicks, last sent newsletter's engagement summary, current cold-count, current active-sending count (so admin sees "we sent X today" at a glance from the hub home). Reuses NewsletterRepository + SubscriberRepository + TrackingEvent queries already in place — mostly a rendering job + caching layer for the heavier per-list / per-category rollups. Pairs with the per-recipient drawer enhancement (show per-row opens/clicks counts; query already returns them, drawer JS just doesn't render them yet).
...
- **Newsletter tracking — ✅ implemented (step 9), pending release.** Working tree carries the full step-9 implementation: image rewriter + open-pixel fallback + link rewriter (Tracking\Pipeline), HMAC-SHA256 token signer (Support\TrackingToken), per-newsletter asset + link side-tables (Schema v8) with their repositories, public REST endpoints at `/lrob-etk/v1/nl/track/{img,click}/{token}` (RestController) with anonymized IP storage (/24 IPv4, /48 IPv6) + opt-in UA, counter bumps on the newsletters companion + per-subscriber lifetime stats (TOTAL_OPENED/CLICKED/SENT, SENDS_SINCE_ENGAGEMENT, LAST_ENGAGEMENT_AT) mirrored to user_meta for WP users, daily retention cron (Tracking\RetentionCron, `lrob_etk_nl_tracking_retention_days` default 365), Cold-subscribers sub-tab in SubscribersPage backed by `SubscriberRepository::list_cold()` + `count_cold()`, Settings panel exposing cold_threshold (default 5) / engagement_counts_opens (default false — Apple MPP poisons opens) / tracking_retention_days. Materializer bumps sender-side lifetime stats per recipient at materialize time. Per-newsletter `META_TRACK_OPENS` / `META_TRACK_CLICKS` toggles already gate each rewriter pass independently. Test-send path bypasses the pipeline (different code path in SendAjaxController::handle_test_send, never enters SendLoop). Still pending follow-up: per-newsletter tracking detail report (per-asset / per-link breakdown), bulk-unsubscribe action on the Cold tab, auto-cleanup cron (auto-unsubscribe past threshold). Original design spec preserved below for reference. Three rewriter passes at send time:
