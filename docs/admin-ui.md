...
| Surfaces | `--etk-card-bg` (solid navy card `#0a2236`), `--etk-input-bg` (field fill, lighter than card), `--etk-soft` (hover / secondary-button fill), `--etk-page-bg` (#wpcontent backdrop — dark navy `#051826`) + `--etk-page-bg-image` (the gradient), `--etk-heading` (brand-blue titles) |
...
| Spacing | `--etk-space-{1..9}` (4 / 8 / 12 / 18 / 28 / 36 / 44 / 52 / 64px). Small steps `1..3` (4/8/12) keep tight internal gaps; `4+` drive card/section breathing (bumped from 16/24/32 for an airier feel). `7..9` = large empty-state / hero padding. Reference the token, never the literal. Sub-4px micro-nudges (1/2/3px), negative pull-ins, and icon-clearance offsets (e.g. `padding-right:22/26px`) stay literal by design. Density-tunable — this scale is the airiness knob. |
| Text sizes | `--etk-text-{xxs,xs,lg,xl,2xl}` = 10 / 12 / 16 / 22 / 30px. **The base body size is 14px, set explicitly on `.lrob-etk` (bumped above WP's 13px default for an airier feel) — normal-flow text inherits it, so DON'T write a `font-size` on plain text.** Only deviations get a token: `xs`=secondary/muted (captions, slugs, helper, table heads, badges, pills); `xxs`=micro glyphs (sort arrows, badge numerals); `lg`=titles; `xl`=section/CTA headings + big counts; `2xl`=hero stat numbers. Display sizes are tokenized too (no bare px in admin chrome). `em` is banned (compounding). `sm`/`md` are retired — don't reintroduce. Density-tunable. |
...
| File | Owns |
|---|---|
| `admin-base.css` | CSS variables (`--etk-*`) = **the single LRob theme**, normalization, page chrome (h1/h2 + the unconditional WP-core dark-surface overrides: `#wpwrap`/inputs/`<option>`/buttons/CF-preview), WP form-table tweaks, generic footer. Scoped under `.lrob-etk`. |
...
## Theme — single LRob "Deep Glass" theme

The plugin ships **one** admin theme. There is **no switcher, no light/dark toggle, no `prefers-color-scheme`, no localStorage, no `data-etk-theme` attribute** (all removed). It deliberately matches the lrob.fr website so plugin + site share one identity. A "theme" here is just the token values in `admin-base.css` — there is no separate theme file. (The frontend `--lrob-etk-cf-*` form theming is a separate system — never cross the two.)

- **Where it lives:** the `--etk-*` colour tokens in the `.lrob-etk` block of `admin-base.css` *are* the theme — dark navy surfaces, amber `#ffb700` accent (black `--etk-on-accent`), brand-blue `#4fb3ec` headings (`--etk-heading`). Full colour chart: [lrob-palette.md](./lrob-palette.md); design mockup: `lrob-theme-mockup.html`.
- **WP-core chrome (unconditional, in `admin-base.css`):** because the surface is always dark, the WP elements that ignore our tokens are repainted explicitly — the `#wpwrap`/`#wpcontent`/`#wpbody-content` backdrop (page-bg `#051826` + the `--etk-page-bg-image` 160° gradient; **literals** there because `#wpwrap` sits outside `.lrob-etk` and can't read the token), native `input`/`select`/`textarea` + `<option>` popups (the `<option>` rule fixes light-on-light dropdowns), headings (`h1`/`h2`/`.lrob-etk-page-title` → `--etk-heading`), `.button`/`.button-primary`/nav buttons, and the in-admin contact-form preview (`--lrob-etk-cf-*` remap so it's legible).
- **Surfaces layered for depth:** page `#051826` < card `#0a2236` < input `#0f2d44` / soft `#11314a`. Cards are **solid** — the shared `--etk-card-bg` feeds 40+ surfaces with no `backdrop-filter`, so translucency would muddy them; the frosted-glass look is a possible later opt-in on the main `.lrob-etk-card` primitive only.
- **Accessibility:** text contrast audited — AA, AAA on most pairs; idle control borders set to `rgba(255,255,255,.40)` (`--etk-line-strong`) to clear WCAG 1.4.11 (≥3:1 non-text). Colorblind-safe status cues + further ratio work fold into the v0.5.x accessibility pass (the once-planned Contrast theme was dropped — it only made sense behind a switcher).
...
Escape, backdrop click, ×, and Cancel all resolve `false`. Primary button resolves `true`. The dialog container is focused on open (not a button) — no stray focus ring, and Enter can't confirm unless the OK button is explicitly focused. The danger variant's OK button uses `.lrob-etk-btn--danger-solid` (solid `--etk-danger`, white text).
...
- **`.lrob-etk-combo` + `-input` + `-toggle` + `-menu`** — input+dropdown shell. The recipient-list row uses the same input-shell idiom; the `<datalist>` ban applies. **Focus** = the wrapper's white border strengthens (`border-color:#fff` + 1px `--etk-line-strong` ring); the inner `<input>` is forced borderless/shadowless so WP's blue focus glow can't bleed through. Inputs/selects/textareas + `.lrob-etk-select` follow the same white-border focus treatment (set in admin-base / components). A menu option may carry an optional `opt.mark` (`'ok'|'fail'`) → leading coloured dot via `.lrob-etk-combo-mark` + `state--on/fail`.
...
- **`.lrob-etk-state--{on,off,fail,pending,info}`** — the **single, shape-agnostic colour source** for every status indicator (success/neutral/danger/warning/accent). Combine with a shape class: `.lrob-etk-status` (generic pill), `.lrob-etk-nl-status` (uppercase pill), `.lrob-etk-nl-recipients-badge`, result/message boxes, etc. — e.g. `class="lrob-etk-nl-status lrob-etk-state--on"`. PHP picks the modifier via `Admin\StatusPill::state_class($value)`; JS reads the same map (localized as `…statusModifiers`) so server- and client-rebuilt pills never drift. **Never re-declare status colours inside a module** — point the shape at `state--*`. (Not yet migrated: `test-result`, `card-status`, banners, `conn-test` still carry their own colour blocks — finish in the next dedup sub-pass.)
...
- **`.lrob-etk-switch-track`** — single track + knob primitive consumed by every switch wrapper (page-toggle, inline-switch, section-switch, retention-toggle). One size (32×18 track, 14×14 knob). The label wrapper owns flex layout + label styling; the track owns dimensions. Checked state is **amber** (`--etk-accent`), not green — matches the LRob theme.
